...
def detect_point_and_ask_user(disp_image,segmented_img,head_point_left,head_point_right,detector_fn_lef,detector_fn_rig):
	...
	# AV: if users have chosen different points (i.e. click has been registered on the image)
	if len(points) != 0:
		left = points[0]
		right = points[1]
		disp_image=temp
...
def measure_distance(segmented_image,segmented_arm_image,arm_spread_image,waist_chest_image,image,metre_pixel_x,metre_pixel_y):
	# print metre_pixel_x
	# print metre_pixel_y
	...
	# AV: lambda function to detect left and right fall
	# 	  detects fall from the tip of the head, since the slope of the neck would be the steepest
	...
	# AV: lambda function to detect left and right shoulder
	#	  detects fall from the left and right neck points
...
def main():
	...
	waist_chest_image = cv2.imread(args["image3"])
	...
	measure_distance(segmented_image,segmented_arm_image,arm_spread_image,waist_chest_image,image,metre_pixel_x,metre_pixel_y)
